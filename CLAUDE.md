...
│   ├── common/                # http / snapshot / audit_overrides / eligibility_text / coupon_form / blackout / geocode
│   ├── schema/                # dataclasses: Library / Attraction / Pass / Branch / Audit + enums
│   ├── sources_v2/            # v2 爬虫(替代旧 sources/)
│   │   ├── assabet/           # 52 馆,catalog + availability + policies
│   │   ├── libcal/            # 5 馆,catalog + availability + policies + branches(BPL)
│   │   ├── museumkey/         # 2 馆(Cohasset+Hingham),catalog + policies(无 availability)
│   │   └── attractions/       # pages.py + llm_extract.py + visitor_eligibility/reservation/prices/hours 请求生成器
│   ├── build/                 # raw + overrides → structured/(libraries/attractions/passes/branches/validate)
│   └── _legacy/               # 旧 sources/(归档,只读)
