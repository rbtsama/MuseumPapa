...
| 距离用户家的开车时间 | ✅ 已有 | OSM Nominatim geocode + haversine 直线距离;75/104 景点 + 55/59 图书馆有坐标(plan-1 完成) |
| 景点原价(划线价) | ⚠️ 部分 | plan-1 抓 54/104 静态/JS-rendered HTML 里的价格;~50 个用 JS booking widget 没静态价,前端 fallback 不显示划线 |
| 景点 hero image | ⚠️ 部分 | plan-1 抓 47/104 og:image meta;无图的 fallback `data/static/placeholders/<category>.svg` |
